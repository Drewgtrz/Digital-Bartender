...
// this fn will get drink by id when user clicks an item from the list
function getById(id) {
  ...
  fetch(url)
    ...
    .then(function (data) {
      ...
      displayDrinkItem(data, displayContainer);
    })
    .catch((err) => console.log(err));
}
// *********************************************************************
...
function DisplayList(items, wrapper, rows_per_page, page) {
    ...
    document.getElementById(itemId).addEventListener('click', function(){getById(itemId)});
	}
}
...
function displayDrinkItem(data, wrapper) {
  ...
  <ul>`;
    ...
    for (let index = 1; index < 16; index++) {
        ...
        if (obj[`strIngredient${index}`] == null || obj[`strIngredient${index}`] == "") {
            break;
        }
